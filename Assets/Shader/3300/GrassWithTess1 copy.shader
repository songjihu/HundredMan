Shader "HundredTalentsProgram/3300TSandGs/GrassWithTessShader2"
{
    Properties
    {
        [Header(Shading)]
    ...
    void geo(triangle VertexOutput IN[3] : SV_POSITION, inout TriangleStream<geometryOutput> triStream){
        ...
        float3x3 facingRotationMatrix = AngleAxis3x3(rand(pos) * UNITY_TWO_PI, float3(0, 0, 1));
                                                                // 向前弯曲矩阵
        float3x3 bendRotationMatrix = AngleAxis3x3(rand(pos.zzx) * _BendRotationRandom * UNITY_PI * 0.1, float3(-1, 0, 0));
                                                                // 空间变换矩阵（从切线空间变换到本地空间）
        ...
        float disFactor = clamp(distance(wPos, _TestLocation),0,100) ;
        disFactor = disFactor < 20 ? 0.1 : 1;
        height = height * disFactor ; 
